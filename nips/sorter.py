...
class Sorter(object):
    ...
    def __init__(self, db_uri, db_name, stage_path, sort_path, preserve_path, sleep_time, tempdir):
    ...
    def run(self):
        while self.alive:
            ...
            if stage_files:
                ...
                log.info('Inspecting  %s' % stage_filename)
                with nimsutil.TempDir(dir=self.tempdir) as temp_dirpath:
                    for sort_dataset, sort_filepaths in self.extract_datasets(stage_filepath, temp_dirpath):
                        log.info('Sorting     %s' % ', '.join([os.path.basename(sfp) for sfp in sort_filepaths]))
                ...
                log.info('Done        %s' % stage_filename)
            else:
    ...
    def extract_datasets(self, stage_filepath, temp_dirpath):
        ...
        for sort_filename in sort_filenames:
            ...
            if dataset:
                ...
                log.warning('Cannot sort %s of %s' % (sort_filename, stage_filename))
...
if __name__ == '__main__':
    ...
    sorter = Sorter(args.uri, args.db, args.stage_path, args.nims_path, args.preserve, args.sleeptime, args.tempdir)
