...
class Sorter(object):
    ...
    def __init__(self, db_uri, stage_path, nims_path, dir_mode, sleep_time, log):
    ...
    def sort_file(self, filepath):
        ...
        dataset = self.mr_data_factory.dataset_at_path_for_file(self.nims_path, filepath)
        if dataset:
    ...
    def sort_directory(self, dirpath, filenames):
        ...
        dataset = self.mr_data_factory.dataset_at_path_for_file(self.nims_path, os.path.join(dirpath, filenames[0]))
        if dataset:
...
class MRDataFactory(object):
    ...
    def dataset_at_path_for_file(self, nims_path, filepath):
        """Return instance of appropriate MRIDataset subclass for provided file."""
        for dataset_class in self.dataset_classes:
            dataset = dataset_class.at_path_for_file_and_datatype(nims_path, filepath)
            if dataset: break
        return dataset
...
if __name__ == '__main__':
    ...
    sorter = Sorter(args.db_uri, stage_path, nims_path, args.dirmode, args.sleeptime, log)
