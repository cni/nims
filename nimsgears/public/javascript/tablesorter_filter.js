...
(function($) {
  $.extend({
    tablesorterFilter: new function() {
	  ...
	  if(container.val() == null) continue;
          var phrase = jQuery.trim(container.val().toString()).replace(/\s+/g, ' ');
          if(phrase.length != 0) {
      ...
      this.construct = function() {
        ...
        return this.each(function() {
          ...
          function checkInputBox(inputBox, override) {
              var value = "";
		for (x=0;x<inputBox.length;x++)
		{
		    if (inputBox[x].selected)
		    {
			if(value == "")
				value = inputBox[x].value;
			else
				value = value + "," + inputBox[x].value;
